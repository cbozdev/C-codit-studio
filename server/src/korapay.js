 ...
 * `data` object (only), signed with the webhook/secret key, must match the
 * `x-korapay-signature` header. Without this check, anyone could POST a
 * fake "payment succeeded" request straight at our webhook endpoint and
 * credit themselves free credits.
 */
export function verifyWebhookSignature(dataObject, signatureHeader) {
  if (!process.env.KORAPAY_WEBHOOK_SECRET || !signatureHeader) return false;
